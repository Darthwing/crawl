...
class Job(MutableMapping):
    ...
    def print_unknown_warnings(self, s):
        for key in s:
            if key not in self.YAML_MAIN_FIELDS:
                print("jobs_gen.py warning: Unknown field '%s' in job %s"
    ...
    def from_yaml(self, s):
        ...
        if self.starting_job != has_recommended_species:
            raise ValueError('recommended_species must not be empty (or'
...
ALL_SKILLS = ('SK_FIGHTING', 'SK_SHORT_BLADES', 'SK_LONG_BLADES', 'SK_AXES',
    ...
    'SK_CROSSBOWS', 'SK_THROWING', 'SK_ARMOUR', 'SK_DODGING', 'SK_STEALTH',
    'SK_SHIELDS', 'SK_UNARMED_COMBAT', 'SK_SPELLCASTING', 'SK_CONJURATIONS',
    'SK_HEXES', 'SK_CHARMS', 'SK_SUMMONINGS', 'SK_NECROMANCY',
...
def main():
    parser = argparse.ArgumentParser(description='Generate jobs-data.h')
